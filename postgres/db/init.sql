...
CREATE TABLE IF NOT EXISTS Venta (
    ...
    id_empleado INTEGER NOT NULL REFERENCES Empleado (id_empleado)
        ON DELETE RESTRICT,
    PRIMARY KEY (id_venta, tipo_pago)
) PARTITION BY LIST (tipo_pago);

CREATE TABLE venta_contado PARTITION OF Venta
    FOR VALUES IN ('contado');

CREATE TABLE venta_financiado PARTITION OF Venta
    FOR VALUES IN ('financiado');
...
CREATE TABLE IF NOT EXISTS Financiacion (
    ...
    tipo_pago VARCHAR(24) NOT NULL,
    FOREIGN KEY (id_venta, tipo_pago)
        REFERENCES Venta (id_venta, tipo_pago)
            ON DELETE CASCADE
);
